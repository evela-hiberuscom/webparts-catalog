# Security Compliance Review

SPFx webpart project for **GOV-23 — Establecer revisiones periódicas de seguridad y cumplimiento**.

## Scope

Los riesgos de permisos, contenido, owners y sharing cambian continuamente; una auditoría puntual no basta.

## MVP

Calendario de revisiones recurrentes por criticidad: mensual para críticos, trimestral para medios, semestral para bajos.

## Current implementation

- Independent SPFx 1.22.2 project under `projects/sharepoint-governance-webparts/security-compliance-review/`.
- React 17 + Fluent UI v8 dashboard with loading, empty, error, mock and backend-required states.
- Local mock repository, service layer, hook and unit tests.
- No real tenant mutation, destructive operation or direct Graph/CSOM/Purview call is implemented in the client.

## Build

```powershell
npm ci --ignore-scripts
npm run build
```

## Backend integration

The webpart requires a backend/API to replace mock data with audited tenant evidence. See `docs/design-technical/security-compliance-review.technical.md` and the global backlog in `../_governance/docs/backlog/backend-required-work.md`.
