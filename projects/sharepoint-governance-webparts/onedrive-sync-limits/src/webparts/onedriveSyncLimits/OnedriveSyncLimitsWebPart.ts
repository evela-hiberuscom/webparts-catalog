...
import * as strings from 'OnedriveSyncLimitsWebPartStrings';
import { OnedriveSyncLimitsDashboard } from './components/OnedriveSyncLimitsDashboard';
...
export interface IOnedriveSyncLimitsWebPartProps {
  title: string;
  subtitle: string;
  maxItems: number;
  showDetails: boolean;
...
export default class OnedriveSyncLimitsWebPart extends BaseClientSideWebPart<IOnedriveSyncLimitsWebPartProps> {
  ...
  public render(): void {
    ...
    const element = React.createElement(OnedriveSyncLimitsDashboard, {
