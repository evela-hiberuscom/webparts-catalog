...
export default function CountdownStatePanel(props: ICountdownStatePanelProps): React.ReactElement {
  return (
    <Stack tokens={{ childrenGap: 16 }} className={styles.statePanel}>
      {props.loading ? <Spinner size={SpinnerSize.large} label={props.message} /> : null}
      ...
      {props.actionLabel && props.onAction ? <PrimaryButton text={props.actionLabel} onClick={props.onAction} /> : null}
    </Stack>
  );
}
