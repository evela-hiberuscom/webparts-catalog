...
export default class ShiftsGuardsAvailabilityWebPart extends BaseClientSideWebPart<IShiftsGuardsConfiguration> {
  private _service: ShiftsGuardsService | undefined;
  public render(): void {
    const repository = new ShiftsGuardsRepository({ fetchClient: window.fetch.bind(window), spHttpClient: this.context.spHttpClient, webAbsoluteUrl: this.context.pageContext.web.absoluteUrl });
    this._service = new ShiftsGuardsService(repository);
    const element = React.createElement(ShiftsGuardsAvailability, { configuration: { dataSourceType: this.properties.dataSourceType || 'StaticConfig', listTitleOrUrl: this.properties.listTitleOrUrl || '', maxItems: this.properties.maxItems || 10 }, service: this._service, title: strings.WebPartTitle });
